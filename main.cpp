...
void loadData() {
    string filename;
    cout << "Masukkan nama file data (ex: papers.csv/txt): ";
    cin >> filename;
    cin.ignore();
    ...
    while (getline(file, line)) {
        ...
        if (tokens.size() >= 5) {
            Paper paper;
            paper.source = tokens[0];
            paper.topic = tokens[1];
            paper.title = tokens[2];
            paper.author = tokens[3];
            paper.year = stoi(tokens[4]);
...
void buildIndexes() {
    ...
    indexesBuilt = true;
    cout << "Index berhasil dibangun dalam " << duration.count() / 1000.0 << " ms" << endl;
}
